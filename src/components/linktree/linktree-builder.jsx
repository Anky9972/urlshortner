...
const LinkTreeBuilder = () => {
  ...
  // Reset dirty flag when loading completes (state was just populated from server)
  useEffect(() => {
    if (!isLoading) {
      setIsDirty(false);
    }
  ...
  const loadLinkTree = async () => {
    ...
    try {
      ...
      if (trees && trees.length > 0) {
        ...
        setTreePassword('');
        setPublishAt(tree.publishAt ? tree.publishAt.slice(0, 16) : '');
        setUnpublishAt(tree.unpublishAt ? tree.unpublishAt.slice(0, 16) : '');
        setCustomDomainId(tree.customDomainId || null);
        setTeamId(tree.teamId || null);
        setProfile({
  ...
  const saveLinkTree = async () => {
    ...
    try {
      if (linkTreeId) {
        // Update existing
        const updated = await updateLinkTree(linkTreeId, {
        ...
        await bulkUpdateLinks(linkTreeId, linksWithUuid.map((l, i) => ({
          id: typeof l.id === 'string' && !l.id.startsWith('temp-') ? l.id : undefined,
          title: l.title,
          url: l.url || '',
          icon: l.icon,
          isActive: l.isActive !== false,
          ...
          activatesAt: l.activatesAt || null,
          deactivatesAt: l.deactivatesAt || null,
          abVariantUrl: l.abVariantUrl || null,
          abVariantTitle: l.abVariantTitle || null,
          abWeight: l.abWeight ?? 50,
        })));
      } else {
        // Create new — slug auto-generated by backend if not provided
        const autoSlug = slug || (profile.name || title).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
      ...
      <div className="min-h-screen w-full bg-[hsl(230,15%,5%)]">
        <main className="w-full flex gap-5 lg:p-2 h-full relative">
          ...
          <Sidebar
            ...
            setTreePassword={setTreePassword}
            publishAt={publishAt}
            setPublishAt={setPublishAt}
            unpublishAt={unpublishAt}
            setUnpublishAt={setUnpublishAt}
            customDomainId={customDomainId}
            setCustomDomainId={setCustomDomainId}
            teamId={teamId}
            setTeamId={setTeamId}
          />
